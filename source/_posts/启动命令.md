...
start "Nacos" %WT_PATH% -w 0 nt -d "%NACOS_HOME%\bin" -p "CMD" --title "Nacos" -- "%COMSPEC%" /c "chcp 65001 >nul && %START_CMD% -m standalone"
...
::    call %REDIS_HOME%\%REDIS_EXE% %REDIS_HOME%\%REDIS_CONF%
    wt -w 0 nt -p "CMD" --title "Redis" -- "%COMSPEC%" /c "chcp 65001 >nul && %REDIS_HOME%\%REDIS_EXE% %REDIS_HOME%\%REDIS_CONF%"
) else (
    :: 方式2：无配置文件直接启动
    echo 启动命令: %REDIS_HOME%\%REDIS_EXE% --port %REDIS_PORT%
::    call %REDIS_HOME%\%REDIS_EXE% --port %REDIS_PORT%
    wt -w 0 nt -p "CMD" --title "Redis" -- "%COMSPEC%" /c "chcp 65001 >nul && %REDIS_HOME%\%REDIS_EXE% --port %REDIS_PORT%"
)
...
@echo off
:: chcp 65001 >nul
:: start "Gateway" cmd /c "java -Dfile.encoding=UTF-8 -jar service-gateway-1.7.1.RELEASE.jar --spring.cloud.bootstrap.location=gateway\bootstrap.yml & pause"
:: start "Auth" cmd /c "java -Dfile.encoding=UTF-8 -jar service-auth-1.7.1.RELEASE.jar --spring.cloud.bootstrap.location=auth\bootstrap.yml & pause"

:: 启动Windows Terminal，并在新标签页中运行JAR
wt -w 0 nt -p "CMD" --title "Gateway" -- "%COMSPEC%" /c  "chcp 65001 >nul && java -Dfile.encoding=UTF-8 -Xbootclasspath/a:C:\E\import\gateway -jar C:\E\import\gateway\service-gateway-1.7.1.RELEASE.jar & pause"
wt -w 0 nt -p "CMD" --title "Auth" -- "%COMSPEC%" /c "chcp 65001 >nul && java -Dfile.encoding=UTF-8 -Xbootclasspath/a:C:\E\import\auth -jar C:\E\import\auth\service-auth-1.7.1.RELEASE.jar & pause"
```
