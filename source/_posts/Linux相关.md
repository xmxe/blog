...
cat file1 >> file2 # 把 file1 的文档内容输入file2 这个文档里
```
###### 用户/用户组
...
# 查看linux所有用户组
cut -d : -f 1 /etc/group # -d : 以“：”为分割符进行分割 -f 1 展示第一列
...
```
[Linux 定时任务调度(crontab)，太实用了！](https://mp.weixin.qq.com/s/c91XWEQvr9Axcf0hjvuKgg)
...
ubuntu在/etc/resolv.conf 写入nameserver 114.114.114.114
...
top -d 3 # 表示更新周期为3秒
# top命令进入展示信息后按大写M 表示按照内存大小排序展示 按大写P表示按照CPU使用率进行排序
```
...
ssh-add ~/.ssh/id_rsa 
# ssh A 和ssh B测试ssh本机和远程是否已经免密登录（第一次免密登录需要输入密码，以后不需要）
ssh登陆 ssh root@ip
```
...
# 使用nc命令
nc -vu ip 端口
# -v 输出交互或出错信息，新手调试时尤为有用 -u 指定nc使用UDP协议，默认为TCP
```
...
观察方法出参，返回值，-b在调用之前观察 -e 在方法异常之后观察 -x 指定输出结果的遍历深度 默认为1
...
- [学会 arthas，让你 3 年经验掌握 5 年功力](https://mp.weixin.qq.com/s/7RMjfIYlmskMsifnhIzxgA)
...
**设置交换空间**
```shell
# 1. 在Home目录创建一个大小为16G的swap文件，块大小为1MByte，总共1K个块，也就是总共 1GB
