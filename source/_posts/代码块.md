...
img: http://qungz.photo.store.qq.com/qun-qungz/V51g1j7W00cGjy3O92fU33gLE919gaa6/V5bCQAyOTM0MDQ3NTjIu0RjQ2NMNg!!/800?w5=473&h5=305&rf=viewer_421
---
...
###### **excel 在任一单元格输入  =CELL("filename")  即可查看路径和文件名**
...
- display:inline 内联元素特点：
1. 和其他元素都在一行上；
2. 元素的高度、宽度及顶部和底部边距不可设置；
3. 元素的宽度就是它包含的文字或图片的宽度，不可改变。
- display:inline-block 内联块状元素（inline-block）就是同时具备内联元素、块状元素的特点。
inline-block 元素特点：
1. 和其他元素都在一行上；
2. 元素的高度、宽度、行高以及顶和底边距都可设置
...
float
```
[如何用一行 CSS 实现 10 种现代布局？](https://mp.weixin.qq.com/s/3yToJq5N8-8SXb0nPDL47g)

###### **css隐藏滚动条**

```css
.class::-webkit-scrollbar{ width:0 !important /*display:none;*/}
直接::-webkit-scrollbar{width:0 !important /*display:none;*/}也可以
```
...
/*文章内容自动换行*/
#articleContent {
    /*在长单词或 URL 地址内部进行换行。normal 只在允许的断字点换行（浏览器保持默认处理）。*/
...
庚顿返回一段时间内多个测点相同个数的数据，map中的key为测点，value为集合，返回侧点数量，测点时间都相同
处理成{time:,tag1:value1,tag2:value2,...}

```java
List<Map<String,Object>> list = new ArrayList<>();

Map<String,List<LightModel<Object>>> map = LightRead.readPlotValuesByTimeDif(start,end,tags,1000);

for(Map.Entry<String,List<LightModel<Object>>> entry : map.entrySet()){
	// 测点
	String key = entry.getKey();
    // 值集合
    List<LightModel<Object>> value = entry.getValue();
    // 遍历集合，返回的list的数量与value数量相同
    for(int i = 0;i<value.size();i++){
        HashMap<String,Object> m = i < list.size()-1?list.get(i):new HashMap<String,Object>();
        LightModel<Object> light = value.get(i);
        m.put(key,light.getTagValue());
        if(m.get("time") == null){
        m.put("time",light.getStrTime());
        list.add(m);
    }
  }
}
